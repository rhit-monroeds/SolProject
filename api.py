...
def cex_checkout():
    for wallet in wallets:
        ...
        while 1:
            ...
            api_call = "https://pro-api.solscan.io/v2.0/account/transfer?address=" + wallet + "&activity_type[]=" + CEX_ACTIVITY_TYPE + "&token=" + NATIVE_SOLANA + "&amount[]=10&block_time[]=" + str((datetime.now() - timedelta(hours=12)).timestamp()) + "&block_time[]=" + str((datetime.now()).timestamp()) + "flow=out&page=" + str(pg) + "&page_size=" + str(CEX_PG_SZ)
            pg += 1
...
def wallet_checkout(wallet, check_until):
    ...
    viewed_tokens = {}
    # while flag:
    api_call = "https://pro-api.solscan.io/v2.0/account/transfer?address=" + wallet + "&activity_type[]=" + DEX_ACTIVITY_TYPE + "&token=" + NATIVE_SOLANA + "&amount[]=9.9&block_time[]=" + str(check_until) + "&block_time[]=" + str((datetime.now()).timestamp()) +"&flow=out&page=" + str(pg) + "&page_size=" + str(DEX_PG_SZ)
    # pg += 1
    response = requests.get(api_call, headers=headers)
    if not response:
        raise Exception(f"Non-success status code: {response.status_code}")
